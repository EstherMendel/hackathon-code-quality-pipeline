...
The repository contains both the main metric-extraction code and a small preprocessing/postprocessing folder.

The expected end-to-end order is:

```text
pre_post_processing/
  0. retrieve_gitdata.py
     Devpost project data + GitHub API token
     -> github_repo_cache.sqlite

  1. enrich_dataset.py
     Devpost CSV files + github_repo_cache.sqlite
     -> dataset_github_enriched.db

  2. prerun_filtering.py
     dataset_github_enriched.db
     -> data_filtered.db

main metric-extraction pipeline
  main.py
     data_filtered.db, through the path configured in config.py
     -> project-level metrics database + file-level metrics database

pre_post_processing/
  3. error_filtering.py
     project-level metrics database + file-level metrics database
     -> no_errors.db
```

In other words, scripts `0`, `1`, and `2` prepare the input dataset for the metric-extraction pipeline. The main pipeline then clones repositories, checks out the relevant commits, and extracts static code metrics. Script `3` is run only after metric extraction, because it needs both the project-level metric output and the file-level `file_metrics` table.

### GitHub token

The first preprocessing script retrieves repository metadata from the GitHub API. It reads the token from the `GITHUB_TOKEN` environment variable:

```bash
export GITHUB_TOKEN="your_token_here"
```

The script can technically run without a token, but GitHub's unauthenticated API rate limits are much lower, so a token is recommended for the full dataset.

### Pre/post-processing folder

The folder `pre_post_processing/` contains the dataset preparation and final error-filtering scripts:

| Script | Purpose | Main output |
| ------ | ------- | ----------- |
| `0. retrieve_gitdata.py` | Extracts GitHub repository links from the Devpost project data and retrieves repository metadata through the GitHub API. | `github_repo_cache.sqlite` |
| `1. enrich_dataset.py` | Combines Devpost hackathon, project, participant, and cached GitHub metadata into one enriched SQLite dataset. | `dataset_github_enriched.db` |
| `2. prerun_filtering.py` | Applies the pre-run dataset filters before metric extraction, such as valid GitHub repository, sufficient participants, public repository, non-empty repository, valid timing, and supported language. | `data_filtered.db` |
| `3. error_filtering.py` | Applies the post-run error filtering after metric extraction, using both project-level extraction status and file-level parse failures. | `no_errors.db` |
...
* `pre_post_processing/` — scripts for GitHub metadata retrieval, dataset enrichment, pre-run filtering, and post-run error filtering.
* `main.py` — entry point for running the metric-extraction pipeline.
...
In the full thesis workflow, this database is produced by:

```text
pre_post_processing/2. prerun_filtering.py
```

For example, either configure `DB_PATH` to point to `data_filtered.db`, or write/copy the pre-run filtered database to the path already expected by `config.py`.
...
For a full run from raw Devpost data to the final error-filtered analysis database, run the steps in order:

```bash
export GITHUB_TOKEN="your_token_here"

python pre_post_processing/0.retrieve_gitdata.py
python pre_post_processing/1.enrich_dataset.py
python pre_post_processing/2.prerun_filtering.py

python main.py

python pre_post_processing/3.error_filtering.py
```

Before running `main.py`, make sure `config.py` points `DB_PATH` to the pre-run filtered database created by `2. prerun_filtering.py`.

Run only the metric-extraction pipeline with:
